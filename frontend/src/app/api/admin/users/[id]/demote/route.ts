...
export async function PATCH(
  ...
  if (error) {
    return NextResponse.json({ error: 'Erro ao rebaixar usuário' }, { status: 500 })
  }

  if (!data || data.length === 0) {
    return NextResponse.json({ error: 'Usuário não encontrado ou não é influencer' }, { status: 404 })
  }
